...
from codelets.examples.genesys.compilation_stages.stage_utils import default_tile_heuristic, \
    ...
    for o in cdlt.all_operands:
        ...
        while dm_idx < len(o.data_moves):
            ...
            if op.op_type == "compute":
                ...
                if all([off == 0 for off in dm.offset_map.values()]):
                    snode = hag.get_subgraph_node(dm.src_node)
                    assert snode.get_type() == "StorageNode", f"Expected storage node in offset map, but got {snode.get_type()}.\n" \
                                                              f"Operation: {op.op_str}, Operand: {o.name}"
