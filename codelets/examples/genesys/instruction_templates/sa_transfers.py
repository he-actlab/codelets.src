...
def gpu_scaling_off_chip_transfer(ld_st, buffer_name, hag: ArchitectureNode):
    instructions = []
    ## FIX RULES:
    # LOOP ID >= 14
    ...
    base_shape = f"(program.get_scaled_shape(cdlt, op.operand))"
    if buffer_name != "WBUF":

        ld_st_tabs = f"op.loop_level + len(op.sizes_for_node('{buffer_name}'))"

        loop_iter_str = f"dim_info[1][1] - 1"
        req_size_str = f"op.scaled_strides_iters({base_shape}, {data_width}, {hag.meta_cfg['MERGE_LDST_LOOPS']}, divisor={n_banks}, max_bits=16)[0][-1]"
        ...
        iterable_str = f"enumerate(zip(*op.scaled_strides_iters({base_shape}, {data_width}, {hag.meta_cfg['MERGE_LDST_LOOPS']}, divisor={n_banks}, max_bits={max_bits})))"
        # END CHANGES
        ...
        req_size_str = f"op.scaled_strides_iters({base_shape}, {data_width}, {hag.meta_cfg['MERGE_LDST_LOOPS']}, divisor={n_banks}, max_bits={max_bits}, contiguous=True)[0][-1]"
        ld_str_size = f"op.scaled_strides_iters({base_shape}, {data_width}, {hag.meta_cfg['MERGE_LDST_LOOPS']}, divisor={n_banks}, max_bits={max_bits}, contiguous=True)[0][-1]"
        ...
        instr.set_field_flex_param("NUM_ITERATIONS", f"op.scaled_strides_iters({base_shape}, {data_width}, {hag.meta_cfg['MERGE_LDST_LOOPS']}, divisor={n_banks}, "
                                                     f"max_bits={max_bits},"
...
def off_chip_transfer(ld_st, buffer_name, hag: ArchitectureNode):
    if 'GPU_SCALING' in hag.meta_cfg:
        return gpu_scaling_off_chip_transfer(ld_st, buffer_name, hag)
    instructions = []
    ## FIX RULES:
    # LOOP ID >= 14
    # UNIQUE LOOPS PER DATA
