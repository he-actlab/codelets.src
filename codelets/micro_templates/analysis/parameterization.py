...
from codelets.micro_templates import OperandTemplate, IndexOperandTemplate, ComputeTemplate, TransferTemplate
...
def add_transfers_transfer_op(cdlt: CodeletTemplate, o: TransferTemplate, hag: ArchitectureNode,
                              transferred_sources: Dict[str, int]):
    ...
    if source.name not in transferred_sources and o.dst_op in cdlt.outputs:
        ...
        o.src_op = result
    return transfer_operands


def add_transfers_compute_op(cdlt: CodeletTemplate, o: ComputeTemplate, hag: ArchitectureNode,
                             transferred_sources: Dict[str, int]):
...
def add_transfers(cdlt: CodeletTemplate, hag: ArchitectureNode):
    ...
    for i, o in enumerate(cdlt.ops):
        transfer_operands = []
        if o.op_type == "compute":
            transfer_operands = add_transfers_compute_op(cdlt, o, hag, transferred_sources)
        elif o.op_type == 'transfer' and o.src_op.location is not None:
            transfer_operands = add_transfers_transfer_op(cdlt, o, hag, transferred_sources)
