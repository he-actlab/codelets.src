...
def softmax(hag):
    ...
    with CodeletTemplate("softmax") as cdlt:
        ...
        z1 = cdlt.create_operand_template("z1", OP_DTYPES, [N, C], default_dtype=acc_dtype)
        ...
        with cdlt.loop(C) as c:
            with cdlt.loop(N) as n:
                ...
                # Now, Comptue exp
                cdlt.compute("MUL", [out[n, c], bval], [y[n, c]], target="SIMD")
                cdlt.compute("FLOOR", [y[n, c]], [y[n, c]], target="SIMD")
                cdlt.compute("MUL", [y[n, c], aval], [y[n, c]], target="SIMD")
                cdlt.compute("ADD", [out[n, c], y[n, c]], [out[n, c]], target="SIMD")
                cdlt.compute("ADD", [out[n, c], cval], [out[n, c]], target="SIMD")

                cdlt.compute("MOVE", [out[n, c]], [y[n, c]], target="SIMD")
                cdlt.compute("MUL", [out[n, c], y[n, c]], [out[n, c]], target="SIMD")

                cdlt.compute("MUL", [out[n, c], dval], [out[n, c]], target="SIMD")
                cdlt.compute("ADD", [out[n, c], eval], [out[n, c]], target="SIMD")

                # Next, compute sum for denominator
                cdlt.compute("MOVE", [zero], [mx[n]], target="SIMD")
                cdlt.compute("ADD", [out[n, c], mx[n]], [mx[n]], target="SIMD")
...
def softmax4d(hag):
    ...
    with CodeletTemplate("softmax4d") as cdlt:
        ...
        # Expontential temporaries
        z1 = cdlt.create_operand_template("z1", OP_DTYPES, [N, C, H, W], default_dtype=acc_dtype)
        cdlt.add_temp_operand(z1)
