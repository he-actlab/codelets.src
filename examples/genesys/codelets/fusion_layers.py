                            ...
                            with cdlt.loop(KH) as kh:
                                with cdlt.loop(KW) as kw:
                                    ...
                                    cdlt.compute("MAX", [out[n, c, y, x], max_op],
                                                 ...
                                                 target="SIMD")
                                    cdlt.compute("MAX", [out[n, c, y, x], min_op],
                                                 [out[n, c, y, x]
                                                  ],
                                                 target="SIMD")
                ...
                with cdlt.loop(OH) as y:
                    with cdlt.loop(OW) as x:
                        ...
                        cdlt.compute("MAX", [out[n,oc, y, x], max_op],
                                     ...
                                     target="SIMD")
                        cdlt.compute("MAX", [out[n,oc, y, x], min_op],
                                     [out[n,oc, y, x]
                                      ],
                                     target="SIMD")
                        cdlt.transfer(out, ["VMEM2", "DRAM"])
...
def matmul_add(hag):
    with CodeletTemplate('matmul_add') as cdlt:
        ...
        scaled_out = cdlt.create_operand_template("scaled_out", OP_DTYPES, [B, M, P], default_dtype=OP_DTYPES[2])
        cdlt.add_temp_operand(scaled_out)
        ...
        with cdlt.loop(P) as p:
            with cdlt.loop(B) as b:
                with cdlt.loop(M) as m:
                    out.set_write_destination('VMEM1')
                    scaled_out.set_write_destination("VMEM2")
                    indices = (b,m,p)
                    add_scale_op(cdlt, gemm_out, scaled_out, m0, nshift, indices)
                    cdlt.compute("32FXP_8FXP", [scaled_out[indices]], [out[indices]], target="SIMD")
                    cdlt.transfer(out, ["VMEM1", "DRAM"])
        cdlt.configure('end', 'SIMD')
